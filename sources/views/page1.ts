...
export default class page1 extends JetView {
		...
		return { view: 'form', localId: 'frm', elementsConfig: { labelWidth: 90 },
			rows: [
				{ view: 'toolbar', localId: 'tbr',
					cols: [
						{ view: 'label', template: 'CLEAN: ui.text  on: { onChange: (nv) => this.f1_change(nv) }' +
				...
				},
				{
					cols: [
						{
							rows: [
								{ view: 'combo', name: 'type', label: 'Search Type', value: 1, width: 250,
									options: [
										{ id: 1, value: 'A' },
										{ id: 2, value: 'B' },
									]
